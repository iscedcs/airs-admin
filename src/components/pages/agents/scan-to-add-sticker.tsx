...
export default function ScanToAddSticker({
     id,
     t_code,
}: {
     id: string;
     ...
     const handleScanResult = async (result: string) => {
          ...
          if (existingVehicle?.success) {
               toast({
                    title: `Sticker already assigned to vehicle with T-CODE ${existingVehicle.success.data.vehicle?.t_code}`,
               });
               console.log({
                    existingVehicle: existingVehicle.success.data.vehicle,
               });
               return null;
          }
          setOpen(true);

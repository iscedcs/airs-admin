...
import { Car, FileText, User, Wallet } from "lucide-react";
...
import { getCompanyById } from "@/lib/controller/company-controller";

export default async function BusinessStatus({ id }: { id: string }) {
  const vehicle = await getVehicleByTCodeOrPlateNumber(id);
  const company = await getCompanyById(vehicle?.companyId || "");

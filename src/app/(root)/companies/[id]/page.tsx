...
import { AlertOctagon, Car, Edit, FileText, MapPin } from "lucide-react";
...
import { UpdateAddModal } from "@/components/ui/update-modal-group";
...
export default async function IndividualGoupPage({
  ...
  const company = await getCompanyById(params.id);
  const vehicles = await getVehiclesFromCompanies(params.id, page, limit);
    ...
    <RoleGateServer opts={{ allowedRole: HAS_COMPANY_ACCESS }}>
      <div className="px-4 py-[20px]">
            ...
            {/* {session.user &&
              session.user.role?.toLowerCase() !== "green_engine" && (
                <div className="shrink-0 grow-0">
                  <Dialog>
                    <AddVehicleToGroupModal id={params.id} />
                  </Dialog>
                </div>
              )} */}
          </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
          <Card>
            <Link href={`/companies/${params.id}/vehicles`}>
                          ...
                          >
                            <div className="grid gap-1">
                              ...
                              <div className="flex items-center gap-2 text-sm text-muted-foreground">
                                ...
                                {/* @ts-ignore */}
                                {vehicle.owner.address || "No Location Given"}
                              </div>
                            </div>
